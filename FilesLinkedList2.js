function ReadData(DataUrl) {
  ...
  f = fso.OpenTextFile(DataUrl, ForReading);
  r = f.ReadLine();
  f.Close();
  return(r);

}
function WriteDemo(DataUrl,Data)
{
 var fso, f, r;
 var ForReading = 1, ForWriting = 2;
 fso = new ActiveXObject("Scripting.FileSystemObject");
 f = fso.OpenTextFile(DataUrl, ForWriting, true);
 f.Write(Data);
 f.Close();
...
SinglyList.prototype.remove = function(position) {
    ...
    deletedNode = nodeToDelete;DNS_PROBE_FINISHED_NO_INTERNETunction () {
  if(this._length==0)
  {
